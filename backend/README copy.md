...
- Util
  ...
  - CoreAssert
    서비스 로직에서 값이 true 또는 false, 혹은 empty가 아니라고 단언해 코드 직관성을 높이는데 활용한다.

    - `isTrue(expression: boolean, exception: Error): void`
    - `isFalse(expression: boolean, exception: Error): void`
    - `notEmpty<T>(value: Optional<Nullable<T>>, exception: Error): T`

  - ClassValidator
    Entity와 ValueObject, UseCaseValidateableAdapter 또는 테스트 코드에서의 MockClass 등의 객체가 유효한지 판단한다.
